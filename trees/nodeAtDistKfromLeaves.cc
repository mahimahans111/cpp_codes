#include<iostream>
 ...
 int cnt = 0;
 void helper(TreeNode* root, vector<TreeNode*> &psf, int k){
     if(root == NULL) return;
     
     if(root && !root->left && !root->right){
         psf.push_back(root);
         if(psf.size()-k-1 >= 0 && psf[psf.size()-k-1]!=NULL){
           cnt++; 
        //   cout << psf[psf.size()-k-1]->val << endl;
     ...
     psf.push_back(root);
     helper(root->left, psf, k);
     helper(root->right, psf, k);
     psf.pop_back();
 }
